...
def get_model(usr_args):
    ...
    ckpt_file = f"./policy/DP/checkpoints/{usr_args['task_name']}-{usr_args['ckpt_setting']}-{usr_args['expert_data_num']}-{usr_args['seed']}-{usr_args['config_name']}/{usr_args['timestamp']}/{usr_args['checkpoint_num']}.ckpt"
    
    action_dim = usr_args['left_arm_dim'] + usr_args['right_arm_dim'] + 2 # 2 gripper
    print("ckpt_path:",ckpt_file)
    load_config_path = f'./policy/DP/diffusion_policy/config/{usr_args["config_name"]}.yaml'
