...
class DPRunner:
    ...
    def get_action(self, policy: BaseImagePolicy, observaton=None, state=None):
        ...
        with torch.no_grad():
            ...
            action_dict = policy.predict_action(obs_dict_input,state=state)

        # device_transfer
        np_action_dict = dict_apply(action_dict, lambda x: x.detach().to("cpu").numpy())
        action = np_action_dict["action"].squeeze(0)[:self.n_action_steps]
        new_state = action_dict["state"] 
